...
export default class Actor extends Component {
	...
	componentDidMount() {
		...
		Axios.get(TMDB_BASEURL + `/person/${id}`, {
			headers: {
		...
		Axios.get(TMDB_BASEURL + `/person/${id}/external_ids`, {
			headers: {
				"Content-Type": "application/json;charset=utf-8",
				Authorization: `Bearer ${TMDB_TOKEN}`,
			},
		...
		Axios.get(TMDB_BASEURL + `/person/${id}/combined_credits`, {
			headers: {
				"Content-Type": "application/json;charset=utf-8",
				Authorization: `Bearer ${TMDB_TOKEN}`,
			},
		...
		if (actor === null || socials === null || credits === null) {
			...
			var kfMovies = (it) => {
				...
				for (let index = 0; index < count; index++) {
					element.push(sorted[index]);
				}

				return element.map((elt) => {
					var linkToPage =
						...
						<div className="mt-4">
							<Link to={linkToPage}>
								<img
									src={
										elt.poster_path
											? TMDB_IMGURL + "w185" + elt.poster_path
